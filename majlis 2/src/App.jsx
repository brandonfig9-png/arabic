...
/* =========================================================
   Curriculum — a guided, unlockable path of lessons
   ========================================================= */
const CURRICULUM = [
  {
    id: "u1",
    ...
    accent: "#E0A33E",
    lessons: [
      { id: "u1l1", title: "First letters", kind: "letters", letters: ["alif", "baa", "taa", "thaa"] },
      { id: "u1l2", title: "From the throat", kind: "letters", letters: ["jeem", "Haa", "khaa"] },
      { id: "u1l3", title: "The non-joiners", kind: "letters", letters: ["daal", "dhaal", "raa", "zaay"] },
      { id: "u1l4", title: "S sounds", kind: "letters", letters: ["seen", "sheen", "Saad", "Daad"] },
      { id: "u1l5", title: "Heavy & deep", kind: "letters", letters: ["Taa", "DHaa", "ayn", "ghayn"] },
      { id: "u1l6", title: "Familiar shapes", kind: "letters", letters: ["faa", "qaaf", "kaaf", "laam"] },
      { id: "u1l7", title: "Last letters", kind: "letters", letters: ["meem", "noon", "haa", "waaw", "yaa"] },
      { id: "u1l8", title: "Alphabet review", kind: "letters", mode: "quiz", letters: ["alif", "baa", "thaa", "jeem", "Haa", "daal", "raa", "seen", "Saad", "ayn", "qaaf", "meem", "noon", "haa", "yaa"] },
    ],
  },
  {
    id: "u2",
    ...
    level: "Brand new",
    lessons: [
      { id: "u2l1", title: "Everyday objects", kind: "topic", icon: "word", topic: "common everyday objects (house, door, water, book, key)" },
      { id: "u2l2", title: "Colors", kind: "topic", icon: "word", topic: "basic colors" },
      { id: "u2l3", title: "Numbers 1–10", kind: "topic", icon: "word", topic: "the numbers one to ten" },
      { id: "u2l4", title: "Food & drink", kind: "topic", icon: "word", topic: "simple food and drink words (bread, water, tea, coffee, fruit)" },
      { id: "u2l5", title: "Word review", kind: "topic", icon: "star", topic: "a mix of everyday objects, colors, numbers and food words", focus: "review the First Words unit" },
    ],
  },
  {
    id: "u3",
    ...
    level: "Beginner",
    lessons: [
      { id: "u3l1", title: "Greetings", kind: "topic", icon: "chat", topic: "greetings and goodbyes" },
      { id: "u3l2", title: "Introducing yourself", kind: "topic", icon: "chat", topic: "introducing yourself — your name and where you are from" },
      { id: "u3l3", title: "Family", kind: "topic", icon: "chat", topic: "family members" },
      { id: "u3l4", title: "Please & thank you", kind: "topic", icon: "chat", topic: "polite words and phrases (please, thank you, excuse me, sorry)" },
      { id: "u3l5", title: "People review", kind: "topic", icon: "star", topic: "a mix of greetings, introductions, family and polite phrases", focus: "review the Greetings & People unit" },
    ],
  },
  {
    id: "u4",
    ...
    level: "Beginner",
    lessons: [
      { id: "u4l1", title: "Directions", kind: "topic", icon: "map", topic: "asking for and giving simple directions" },
      { id: "u4l2", title: "At the café", kind: "topic", icon: "map", topic: "ordering food and drink at a café" },
      { id: "u4l3", title: "Shopping", kind: "topic", icon: "map", topic: "simple shopping phrases and asking prices" },
      { id: "u4l4", title: "Time & days", kind: "topic", icon: "map", topic: "telling the time and the days of the week" },
      { id: "u4l5", title: "Out & about review", kind: "topic", icon: "star", topic: "a mix of directions, café, shopping and time phrases", focus: "review the Getting Around unit" },
    ],
  },
  {
    id: "u5",
    ...
    level: "Intermediate",
    lessons: [
      { id: "u5l1", title: "This & that", kind: "topic", icon: "grammar", topic: "using this and that with nouns (haadha, haadhihi)" },
      { id: "u5l2", title: "I want / I have", kind: "topic", icon: "grammar", topic: "saying I want and I have with simple objects" },
      { id: "u5l3", title: "Asking questions", kind: "topic", icon: "grammar", topic: "forming simple questions — what, where, how much" },
      { id: "u5l4", title: "Describing things", kind: "topic", icon: "grammar", topic: "using simple adjectives to describe nouns" },
      { id: "u5l5", title: "Sentence review", kind: "topic", icon: "star", topic: "building simple full sentences from earlier units", focus: "review the Simple Sentences unit" },
    ],
  },
];
...
function LessonPlayer({ node, settings, speak, onClose, onComplete, onQuizDone }) {
  ...
  const load = useCallback(async () => {
    ...
    const system = `You are an Arabic tutor. Create a short, beginner-friendly micro-lesson AND a matching quiz in ${settings.dialect} for a "${lvl}" learner. Topic: "${node.topic}".${node.focus ? " This is a review — draw on common words a learner would already have met. Focus: " + node.focus + "." : ""}
Respond ONLY with a JSON object, no markdown or backticks:
...
function Path({ done, onOpen }) {
  const doneSet = new Set(done);
  const flat = [];
  CURRICULUM.forEach((u) => u.lessons.forEach((l) => flat.push(l.id)));
  const total = flat.length;
  const completed = flat.filter((id) => doneSet.has(id)).length;
  let currentId = null;
              ...
              {u.lessons.map((l) => {
                ...
                const accentStyle =
                  status === "done" || status === "current"
                    ...
                    : undefined;
                return (
                  <div className="mj-noderow" key={l.id}>
                    <div
                      className="mj-nodewrap"
                    ...
                    >
                      ...
                      >
                        {status === "current" && (
                          <span
                            className="mj-node-pulse"
                            style={{ borderColor: u.accent }}
                          />
                        )}
                        {status === "done" ? (
                          <IconCheck />
                          ...
                          <span className="mj-node-glyph" dir="rtl" lang="ar">
                            {(lettersByNames(l.letters)[0] || {}).ar}
                          </span>
                        ) : (
                          <IconFor name={l.icon || "word"} />
                        )}
                      </button>
                      {status === "current" && (
                        <span className="mj-node-flag">START</span>
                      )}
                    </div>
                    <div
...
const TOTAL_LESSONS = CURRICULUM.reduce((n, u) => n + u.lessons.length, 0);

function lessonsDone(progress) {
  return (progress && Array.isArray(progress.done) ? progress.done : []).length;
}
              ...
              {profiles.map((p) => (
                <button key={p.id} className="mj-gate-profile" onClick={() => onChoose(p)}>
                  ...
                  <div className="mj-gate-psub">
                    {lessonsDone(p.progress)} / {TOTAL_LESSONS} lessons
                  </div>
                </button>
              ))}
            </div>
            ...
            <div key={p.id} className={"mj-statcard" + (isMe ? " mj-statcard-me" : "")}>
              <div className="mj-statcard-head">
                ...
                <div className="mj-statcard-id">
                  ...
                  <div className="mj-statcard-sub">active {relTime(p.progress && p.progress.updatedAt)}</div>
                </div>
                {!isMe && onNudge && (
              ...
              <div className="mj-statgrid">
                <div className="mj-stat">
                  ...
                  <div className="mj-statlbl">lessons done</div>
                  <div className="mj-bar">
      ...
      {!ready ? (
        ...
        <>
          ...
          <main className="mj-main">
            {view === "path" ? (
              <Path done={done} onOpen={setActiveLesson} />
            ) : view === "letters" ? (
              <Letters
                settings={settings}
                speak={speak}
